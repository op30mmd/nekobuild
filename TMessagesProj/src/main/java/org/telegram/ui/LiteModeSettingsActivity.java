...
public class LiteModeSettingsActivity extends BaseFragment {
    ...
    private void updateItems() {
        ...
        if (expanded[2]) {
            ...
            if (true || SharedConfig.getDevicePerformanceClass() >= SharedConfig.PERFORMANCE_CLASS_AVERAGE || BuildVars.DEBUG_PRIVATE_VERSION) {
                items.add(Item.asCheckbox(LocaleController.getString("LiteOptionsBlur2"), LiteMode.FLAG_CHAT_BLUR));
            }
            if (Build.VERSION.SDK_INT >= 33 && (true || SharedConfig.getDevicePerformanceClass() >= SharedConfig.PERFORMANCE_CLASS_AVERAGE || BuildVars.DEBUG_PRIVATE_VERSION)) {
